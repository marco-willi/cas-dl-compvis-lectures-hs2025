...
Start with: (1) Extract DINOv3 embeddings → (2) Run k-NN (baseline) → (3) Train linear probe → (4) If gap persists, add LoRA adapters → (5) Only full fine-tune if accuracy plateaus & you have enough data.
:::
