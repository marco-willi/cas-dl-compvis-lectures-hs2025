...
- Deep learning frameworks (PyTorch, TensorFlow) provide automatic differentiation via computational graphs
- Key concepts: tensors, modules, autograd, optimizers
- Pre-trained models enable transfer learning and rapid development
...
- **Tensors**: N-dimensional arrays (like NumPy) with GPU support via `.to(device)`
- **Autograd**: Automatic differentiation with `requires_grad=True` and `.backward()`
- **Modules**: Build networks by inheriting `nn.Module` and implementing `forward()`
- **Optimizers**: Use `optimizer.zero_grad()` before each backward pass to clear gradients
- **DataLoaders**: Use multiple workers to prevent GPU starvation
- **Common Pitfalls**: Forgetting `zero_grad()`, in-place operations breaking autograd, device mismatches

**Quick Training Loop Pattern:**
```python
...
- **Industry & research adoption**: Used by Meta, Tesla, OpenAI, and top ML research labs
...
- **AdamW**: Adam with weight decay, often better for transformers
...
::: {.callout-note}
**Critical**: Always call `optimizer.zero_grad()` before computing gradients! PyTorch accumulates gradients by default, so failing to zero them will cause incorrect gradient updates.
:::
...
**Pitfall 3: In-place operations breaking autograd**
```python
# ❌ Wrong - in-place operations can break gradient computation
x = x * 2  # In-place when x requires grad
x += 5     # In-place

# ✅ Correct
x = x * 2  # Creates new tensor
x = x + 5  # Creates new tensor
```

**Pitfall 4: Not using eval() mode for inference**
```python
