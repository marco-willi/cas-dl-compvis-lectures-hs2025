...
## Quiz: Pros and Cons {.center}

::: {.r-fit-text}

What are the main advantages and disadvantages of diffusion models?
...
**Pros**: High quality, diverse outputs, stable training
...
## Quiz: VAE Properties {.center}

::: {.r-fit-text}

What is the key advantage of VAEs over GANs?

What is a common limitation?
...
**Advantage**: Structured latent space, stable training, can compute likelihood

What is a common limitation?

**Limitation**: Often produces blurry outputs
...
::: {.fragment}
Bimodal distribution the models must learn
:::
...
::: {.fragment}
Different convergence behaviors and failure modes
:::
...
::: {.fragment}
GAN: sharp but mode collapse | VAE: smooth gaps | Diffusion: reliable
:::
...
::: {.fragment}
Models handle complex geometry differently
:::
...
## Quiz: Model Comparison {.center}

::: {.r-fit-text}

Which model is most prone to mode collapse?

Which model tends to fill in gaps between modes?

Which model is most reliable at capturing all modes?
...
::: {.fragment}
Both generator and discriminator receive condition $c$
:::
...
**Answer**: Runs diffusion in compressed VAE latent space (8× smaller), making high-resolution generation practical on consumer GPUs
...
What are three major societal concerns with generative AI?
...
**Answers**:
1. Synthetic data pollution (model degradation)
2. Authenticity challenges (real vs. fake)
3. Embedded biases from training data
...
::: {.incremental}
- **Four families**: Autoregressive, Diffusion, GANs, VAEs
- **Different tradeoffs**: Speed, quality, coverage, likelihood
- **Conditional generation**: Extend with text, images, labels
- **Pre-trained models**: Stable Diffusion enables practical applications
- **LoRA**: Efficient fine-tuning with minimal compute
- **Societal impact**: Pollution, authenticity, bias
:::

## When to Use What?

::: {.incremental}
- **Autoregressive**: Need exact likelihood, can tolerate slow generation
- **Diffusion**: Need highest quality and coverage, have compute budget
- **GANs**: Need fast generation, can tolerate mode collapse risk
- **VAEs**: Need structured latent space, interpretability
:::

## Practical Recommendation

::: {.fragment}
Start with **pre-trained Stable Diffusion** + **LoRA fine-tuning**
:::

::: {.fragment}
Only train from scratch if highly specialized domain
:::
