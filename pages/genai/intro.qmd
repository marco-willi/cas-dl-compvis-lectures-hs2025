...
Let's define generative modeling for images more formally.
...
One of the key considerstions is that one input (the label) should be associated with a diverse range of outputs (see @fig-genai-intro-basics-elephant-prob). Since neural networks are inherently deterministic we need a way to make the generator a **stochastic** function to enable sampling diverse outputs.
