...
def create_interactive_logits_explorer():
    ...
    for name, logits in scenarios.items():
        ...
        button = dict(
            ...
            args=[
                ...
                {
                    "title": f"Scenario: {name} | Max Probability: {max_prob:.3f} | Entropy: {entropy:.3f}"
                    #"annotations[0].text": f"Sum = {sum(probs):.6f}"
                }
            ]
        )
    ...
    fig.update_layout(
        ...
        title=f"Scenario: {initial_name} | Max Probability: {initial_max_prob:.3f} | Entropy: {initial_entropy:.3f}"
    )
