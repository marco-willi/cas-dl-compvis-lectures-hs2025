...
- **Optimize** model performance through systematic hyperparameter tuning (grid search).
:::


::: {.callout-tip title="TLDR Recap" collapse="true"}

**6-Step Training Recipe (Karpathy):**

1. **Know your data** - Inspect distributions, corrupted images, class imbalance, resolution needs
2. **Baselines** - Simple models, fix random seeds, define metrics, train/val/test splits
3. **Overfit first** - Use established architectures (ResNet-50), conservative optimizers (Adam)
4. **Regularize** - More data, augmentation, early stopping, weight decay, transfer learning
5. **Tune hyperparameters** - Random search preferred (Ray Tune, Optuna)
6. **Final optimization** - Ensembling, extended training, AdamW, Mixup/Cutmix, label smoothing

**Key Insights:**

- **Leaky abstractions & silent failures** - ML frameworks hide complexity; bad hyperparameters don't error
- **Human baselines** - Annotator agreement sets performance upper bounds (96.6% in Snapshot Serengeti)
- **Data issues** - Handle corrupted images, class imbalance (65% deer), ambiguous labels, resolution requirements
- **Principle: "Don't be a hero"** - Use established architectures, not novel inventions

**Regularization:** More data (best), augmentation (AugLy/Albumentations/Kornia), synthetic data, early stopping, weight decay: $\theta_{t+1} = \theta_t (1 - \lambda) - \eta \nabla J(\theta)$, transfer learning

**Debugging:** Fix seeds, start simple, input-independent baseline (learns class distribution), overfit single batch, visualize inputs before `y_hat = model(x)`, track fixed samples

**Advanced:** Double descent (test error ↓ again after overfitting), grokking (improvement after plateau)

**Tools:** Hugging Face, timm, Weights & Biases, Neptune AI, Hydra, DVC
...
::: {.callout-note title="Quiz: What would you expect?" collapse="true"}
...
1. **k-NN baseline**: Freeze backbone, classify with nearest neighbors (fastest validation)
2. **Linear probe**: Freeze backbone, train only classification head
3. **Partial fine-tune**: Unfreeze last few layers + head
4. **Full fine-tune**: Unfreeze all layers (only if data >> 10k samples)
