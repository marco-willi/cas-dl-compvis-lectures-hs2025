...
We will see that Convolutional Neural Networks (CNNs) are much better suited to model images than MLPs. Unlike MLPs, CNNs explicitly assume that inputs (e.g., pixels) that are close together need to be considered together and that information is locally correlated. Certain properties are thus directly embedded in the architecture of CNNs (inductive biases), enabling them to exploit these properties efficiently (for example, with fewer model parameters).
...
**Question**: Can an MLP learn to detect the same pattern at different spatial locations? If yes, how? See @fig-cnns-mlp-pattern-shift.
